...
class AddPost extends Component {
  ...
  handleMainUpload = e => {
    ...
    const uploadTask = storage.ref(`images/${randomMain}/${imageName}`).put(image);
    uploadTask.on(
      "state_changed",
      snapshot => {
        // progress function ...
  ...
  handleSubmit = e => {
    ...
    if (isValid) {
      const { year, make, model, vin, url, urls, mainUrl, imageName, random, randomMain, file, price, odometer, transmition, cylinders, description, note } = this.state;
